...
/*just test the matcher*/
int demo_matcher(int argc, char* argv[])
{
    ...
    std::cout << "leftName value: " << leftName << std::endl;
    if(img1.empty())
    {
    ...
    double matches_distance_threshold = 0.2f;
    std::cout << "Good matces count " << count_good_matches( matches, matches_distance_threshold) << " threshold " << matches_distance_threshold << std::endl;

    return EXIT_SUCCESS;
}

int run_lfovs(int argc, char* argv[])
{
    //handle input
    const char* keys =
            "{ h help        |            | print help message    }"
    ...
    if(verbose)
        std::cout << "input video:" << input_video_path << std::endl;
    ...
    while(is_running)
    {
        ...
        //wait until the vector is filled
